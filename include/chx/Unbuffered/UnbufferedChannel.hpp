...
public:
  Channel() : chx::Channel<T>(), data_(std::make_shared<DataHandler<T>>()) {}
  ~Channel() = default;
...
private:
  ...
  std::shared_ptr<DataHandler<T>> data_;
};
...
std::expected<void, Error> Channel<T>::send_(U &&value) {
  ...
  this->data_->sender_entrance.wait(
      lock, [&] { return !data_->value_set || data_->closed; });
  if (data_->closed) {
...
template <typename T> std::expected<T, Error> Channel<T>::receive() {
  std::unique_lock lk(data_->mutex);
  this->data_->receivers_waiting++;
  this->data_->receiver_entrance.wait(
      lk, [&] { return data_->value_set || data_->closed; });
  this->data_->receivers_waiting--;
  if (data_->closed && !data_->value_set) {
    return std::unexpected("channel closed");
  }

  T value_read = std::move(*data_->slot);
